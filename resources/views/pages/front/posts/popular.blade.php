...
@section('meta_description', $data['meta_description'] ?? 'Blog of zakialawi.my.id website')
@section('meta_keywords', $data['meta_keywords'] ?? 'Zaki alawi, Blog')

@section('og_title', $data['og_title'] ?? 'Blog | zakialawi.my.id')
@section('og_description', $data['og_description'] ?? 'Blog of zakialawi.my.id website')
...
<x-app-front-layout>
    ...
    <section class="fluid text-dark dark:text-dark-light container px-6 py-10 md:px-4">
        <div class="mb-6 text-3xl font-semibold">
            <h2>{{ request()->has('search') && request()->get('search') != '' ? 'Search Result' : 'Popular Post' }}</h2>
