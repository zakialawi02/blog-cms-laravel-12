<article {{ $attributes->merge(['class' => 'flex flex-col']) }}>
    <img class="h-64 w-full rounded-lg object-cover object-center lg:h-72" src="{{ asset($article->cover) }}" alt="{{ $article->title }}" loading="lazy" onerror="this.onerror=null;this.src='{{ asset('assets/img/image-placeholder.png') }}'">
